...
import { Insight, ReportMetadata, Action } from "../types/insight.js";
import { OutputTarget } from "./output.interface.js";

const SEVERITY_ORDER: Record<string, number> = { alert: 0, action: 1, suggestion: 2, info: 3 };
...
function formatInsight(insight: Insight): string {
  ...
  if (insight.evidence.length > 0) {
    ...
    for (const e of insight.evidence) {
      if (e.metric) lines.push(`- ${e.metric}`);
      if (e.snippet) lines.push(`- \`${e.snippet}\``);
      if (e.sessions?.length) lines.push(`- Sessions: ${e.sessions.join(", ")}`);
    }
  }
...
export function formatMarkdownReport(
  ...
  const severities = [...grouped.keys()].sort(
...
export class MarkdownOutput implements OutputTarget {
  ...
  async publish(insights: Insight[], metadata: ReportMetadata): Promise<void> {
