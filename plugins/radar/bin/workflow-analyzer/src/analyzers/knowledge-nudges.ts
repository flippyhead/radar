...
export class KnowledgeNudgesAnalyzer implements Analyzer {
  ...
  async analyze(input: AnalyzerInput): Promise<Insight[]> {
    ...
    const sessionPrompts: { sessionId: string; prompts: string[] }[] = [];
    ...
    if (sessionPrompts.length < MIN_SESSIONS_FOR_REPETITION) return [];
