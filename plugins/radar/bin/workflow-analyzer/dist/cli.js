...
program
    ...
    if (opts.since)
        config.analysis.lookbackDays = parseInt(opts.since, 10);
    if (opts.llm)
        config.llm.provider = opts.llm;
...
program
    ...
    const projectsBasePath = config.sources["claude-code"]?.path || "~/.claude/projects";
    ...
    if (opts.output) {
        ...
        let summary = `Scanned ${result.projectCount} projects, ${result.packageCount} packages, ` +
...
function buildAnalyzers(config) {
    const analyzers = [];
    const enabled = config.analyzers;
    if (enabled["root-cause"])
        analyzers.push(new RootCauseAnalyzer());
    if (enabled["direct-automation"])
        analyzers.push(new DirectAutomationAnalyzer());
    if (enabled["decision-support"])
        analyzers.push(new DecisionSupportAnalyzer());
    if (enabled["knowledge-nudges"])
        analyzers.push(new KnowledgeNudgesAnalyzer());
    return analyzers;
}
